    ...
    for x in range(0, aLen):
        try:
            if arr[x] == '(' or arr[x] == "(": #If if finds a bracket/parentheses
                ...
                bracAns = getProd(bracAns["array"], temp, bracAns["text"], bracAns["old"]) #runs the product function
                print("Within Brackets(Prod): ", bracAns)
                bracAns = getQuot(bracAns["array"], temp, bracAns["text"], bracAns["old"]) #runs the quotient function
                print("Within Brackets(Quot): ", bracAns)
                bracAns = getSumOrDiff(bracAns["array"], temp, bracAns["text"], bracAns["old"]) #runs the "addition or subtraction" function
